...
export default function OAuthCallback() {
  ...
  const exchangeSupabaseToken = useAuthStore((state) => state.exchangeSupabaseToken);
  ...
  useEffect(() => {
    const handleOAuthCallback = async () => {
      try {
        ...
        if (intent === 'signup') {
          ...
          console.log('[OAuth Callback] Signup intent - routing to create-profile');
          router.replace('/(auth)/create-profile');
        } else {
          // Validate existing user status for login intent
          console.log('[OAuth Callback] Login intent - checking onboarding status');
    ...
    handleOAuthCallback();
  }, [params, exchangeOAuthCode, exchangeSupabaseToken, checkOnboardingStatus, router]);
